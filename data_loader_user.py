...
def load_user_data_df(file_path='yelp_academic_dataset_user.json'):
    with open(file_path, 'r') as json_file:
        all_attributes = get_all_attributes(json_file)
    with open(file_path, 'r') as json_file:
        data_dict = process_user_data(json_file, all_attributes)
...
def process_user_data(json_file, all_attributes):
    '''
    ...
    for line in json_file:
        ...
        for key_level_1, values_level_1 in json.loads(line).items():
            user_data[key_level_1].append(values_level_1)
